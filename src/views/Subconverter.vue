                ...
                <el-collapse>
                  <el-collapse-item>
                    ...
                    <el-form-item label="节点命名:">
                      <el-input
                        v-model="form.rename"
                        placeholder="举例：`a@b``1@2`，|符可用\转义"
                      />
                    </el-form-item>
                    <el-form-item label="远程设备:">
                      <el-input
                        v-model="form.devid"
                        placeholder="用于设置QuantumultX的远程设备ID"
                      />
                    </el-form-item>
                    <el-form-item label="更新间隔:">
                      <el-input
                        v-model="form.interval"
                        placeholder="返用于设置托管配置更新间隔，单位为天"
                      />
                    </el-form-item>
...
const configScriptBackend =
  process.env.VUE_APP_CONFIG_UPLOAD_BACKEND + "/api.php";
...
const configUploadBackend =
  process.env.VUE_APP_CONFIG_UPLOAD_BACKEND + "/sub.php";
    ...
    return {
      ...
      options: {
        ...
        customBackend: {
          ...
          懒蛋蛋服务: "https://sub.landandan.cn:8443",
          本地服务: "http://127.0.0.1:25500",
        },
        backendOptions: [
        ...
        remoteConfig: [
          {
            ...
            options: [
              ...
              },
              {
                label: "自己家用",
                value:
                  "https://raw.githubusercontent.com/landandan/cdn/main/rules/Self_Use_Home.ini",
              },
              {
                label: "自己使用",
                value:
                  "https://raw.githubusercontent.com/landandan/cdn/main/rules/Self_Use.ini",
              },
            ],
          },
          {
  ...
  methods: {
    ...
    makeShortUrl() {
      ...
      if (this.customShortSubUrl.trim() != "") {
        data.append(
          "shortKey",
          this.customShortSubUrl.trim().indexOf("http") < 0
            ? this.customShortSubUrl.trim()
            : ""
        );
      }
      this.$axios
    ...
    confirmLoadConfig() {
      if (
        this.loadConfig.trim() === "" ||
        !this.loadConfig.trim().includes("http")
      ) {
    ...
    renderPost() {
      ...
      data.append(
        "sdoh",
        encodeURIComponent(this.form.tpl.surge.doh.toString())
      );
      data.append(
        "cdoh",
        encodeURIComponent(this.form.tpl.clash.doh.toString())
      );
    ...
    getBackendVersion() {
      this.$axios
        ...
        .then((res) => {
          ...
          this.backendVersion = this.backendVersion.replace(
            "subconverter",
            "SubConverter"
          );
          let a =
            this.form.customBackend.indexOf("apiurl.v1.mk") !== -1 ||
            this.form.customBackend.indexOf("sub.d1.mk") !== -1;
          let b = this.form.customBackend.indexOf("127.0.0.1") !== -1;
          a
            ? this.$message.success(
                `${this.backendVersion}` +
                  "肥羊负载均衡增强版后端，已屏蔽免费节点池（会返回403），额外支持vless reality+hysteria+hysteria2订阅转换"
              )
            : b
            ? this.$message.success(
                `${this.backendVersion}` + "本地局域网自建版后端"
              )
            : this.$message.success(
                `${this.backendVersion}` +
                  "官方原版后端不支持vless/hysteria订阅转换"
              );
        })
        .catch(() => {
          this.$message.error(
            "请求SubConverter版本号返回数据失败，该后端不可用！"
          );
        });
    },
  },
